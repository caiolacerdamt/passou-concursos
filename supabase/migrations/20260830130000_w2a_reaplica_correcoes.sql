-- ALUNO · reaplica as três correções do W2-A que a 20260825141000 desfez.
--
-- A 20260825141000 se propôs a acentuar quatro literais de `motivo` e, para
-- isso, copiou o corpo da 20260824102000 — que já não era a definição vigente.
-- Entre as duas, a 20260824102100 havia patchado a função em três pontos
-- (`pg_get_functiondef` + `replace`), e o `create or replace` de corpo inteiro
-- devolveu a função ao estado anterior sem que nada acusasse. Só uma das três
-- perdas tinha teste: `ciclo-adaptativo` W2-A, "gera somente em dias
-- declarados", que passou a receber 0 onde espera 1.
--
-- O que volta, na ordem em que aparece no corpo:
--   1. plano vazio fora da agenda — o dia não declarado grava `plano_dia` sem
--      bloco e conta no retorno, em vez de sumir do resultado;
--   2. reserva de um slot para o simulado quando a flag está ligada;
--   3. o desvio de cobertura virgem só vale com `perfil_concurso` ativo.
--
-- Esta migration reescreve o corpo inteiro **com as três correções embutidas**,
-- em vez de repatchar por texto: enquanto a definição vigente só existir como
-- patch sobre outra migration, a próxima cópia de corpo inteiro repete o
-- acidente. A partir daqui, este arquivo é o corpo de verdade da função.
