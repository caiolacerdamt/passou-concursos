...
begin
  ...
  select
    ...
    coalesce((select case when jsonb_typeof(valor) = 'number'
                          ...
                          then (valor #>> '{}')::integer end
                from public.configuracoes_vigentes
               where chave = 'param.m4.teto_blocos_dia'), 6),
    coalesce((select case when jsonb_typeof(valor) = 'number'
                           and (valor #>> '{}') ~ '^[1-9][0-9]*$'
                          then (valor #>> '{}')::integer end
                from public.configuracoes_vigentes
               where chave = 'param.m4.teto_materias_dia'), 3),
    coalesce((select case when jsonb_typeof(valor) = 'number'
                           and (valor #>> '{}') ~ '^[0-9]+([.][0-9]+)?$'
                          then (valor #>> '{}')::numeric end
                from public.configuracoes_vigentes
  ...
  loop
    ...
    v_review_slots := least(
      ...
      v_teto_revisoes,
      greatest(round(v_total_slots * v_pct_revisar)::integer, 0)
    );
    ...
    v_advance_slots := least(
      v_slots_restantes,
      greatest(round(v_total_slots * v_pct_avancar)::integer, 0)
    );
    ...
    v_practice_slots := least(
      v_slots_restantes,
      greatest(round(v_total_slots * v_pct_praticar)::integer, 0)
    );
    ...
    for v_pass in 0..1 loop
      ...
      loop
        ...
        -- Materia nova so entra enquanto o dia nao encheu de materias. Materia ja
        -- usada passa livre: e isso que concentra os blocos em 2-3 materias em
        -- vez de pincelar o edital (ALUNO-08 AC3).
        if not (v_topico.materia_id = any(v_usados_materias))
           and (select count(distinct x) from unnest(v_usados_materias) x) >= v_teto_materias then
          continue;
        end if;
