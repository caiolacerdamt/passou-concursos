-- ALUNO-03 AC1 · correcao apontada pelo Verificador da SPEC 05 (gap G1)
--
-- O AC1 exige que o sistema **exija a causa antes de avancar**, com recusa
-- propria. Na primeira versao quem recusava era o `CHECK
-- causa_obrigatoria_no_treino` da tabela: a resposta era correta, mas chegava na
-- tela como erro de constraint do Postgres, sem motivo nomeado.
--
-- A recusa nao podia morar no modulo TypeScript porque ele **nao conhece o
-- gabarito** (invariante nº4: a verdade e do banco), e "errou no treino" so se
-- sabe depois de comparar a resposta com `questoes.resposta_correta`. O lugar
-- certo e aqui dentro, no instante seguinte a essa comparacao e antes do INSERT
-- — que e literalmente o que o AC pede.
--
-- `validarResposta()` continua existindo no modulo: e a checagem otimista da
-- tela, que ja sabe o tipo da questao e pode pedir a causa sem ida ao servidor.
-- Ela e conveniencia; **esta** e a garantia.
--
-- Segundo conserto (gap G6): no ramo do duplo-clique, item marcado como
-- respondido sem tentativa correspondente devolvia zero linha, e o chamador
-- estourava `TypeError` ao ler `undefined`. Agora e uma excecao nomeada.
...
begin
  ...
  if p_contexto = 'treino' and not v_correta and p_causa is null then
    raise exception
...
comment on function public.registrar_tentativa is
  'Grava uma resposta como linha permanente (ALUNO-01/ALUNO-03). Dedup, snapshot e INSERT sao atomicos por serem uma funcao so. Recusa nomeada `causa_obrigatoria` antes do INSERT quando o aluno erra no treino sem declarar a causa (ALUNO-03 AC1). `security invoker`: a RLS continua valendo dentro dela.';
