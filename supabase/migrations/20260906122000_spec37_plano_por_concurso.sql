-- SPEC 37 - ALUNO-13 AC2 - AD-139
--
-- O plano do dia passa a seguir o concurso **do aluno**. Ate aqui ele juntava a
-- view `raiox_peso_topico`, que casa `perfil_concurso` por `p.ativo` - uma flag
-- global. Dois alunos em concursos diferentes recebiam o mesmo edital.
--
-- Sao tres trocas, e nenhuma outra: (a) o cursor dos alunos passa a resolver o
-- perfil de cada um por `perfil_concurso_do_aluno`; (b) os seis joins da view
-- viram `raiox_peso_do_aluno(v_aluno.user_id)`; (c) o desvio de cobertura
-- virgem pergunta pelo perfil do aluno em vez de perguntar se existe algum
-- perfil ativo no banco. Com `flag.m5.multi_concurso` desligada,
-- `perfil_concurso_do_aluno` devolve o perfil ativo para todo mundo e o
-- comportamento e identico ao de hoje.
--
-- O corpo abaixo e copia do de 20260830130000, que aquele arquivo declarou ser
-- "o corpo de verdade da funcao", com as tres trocas aplicadas por
-- substituicao mecanica. A partir daqui, **este** arquivo e o corpo de verdade:
-- a proxima alteracao copia daqui, nao de antes.
...
begin
  ...
  for v_aluno in
    select p.user_id, p.minutos_por_dia, p.nivel_declarado, p.dias_estudo,
           public.perfil_concurso_do_aluno(p.user_id) as perfil_concurso_id
  ...
  loop
    ...
    select exists (
      select 1
        ...
        join public.raiox_peso_do_aluno(v_aluno.user_id) rx on rx.topico_id = r.topico_id
    ...
    for v_pass in 0..1 loop
      ...
      loop
        ...
        if v_aluno.perfil_concurso_id is not null
